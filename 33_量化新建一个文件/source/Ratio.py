...
class ratio:
    ...
    def __init__(self):
        pass
    def cal_ratio(self,datas:pd.core.frame.DataFrame,day:bool=True):
        assert isinstance(datas,pd.core.frame.DataFrame),'请传入pandas.DataFrame对象，每一列为对应每时段收益率'
        res = []
        # 定义函数求各项回测指标
        for item in datas:
            data = datas[item]
            ...
            # 下跌天数
            num_of_unchange = data[data == 0].count()
            # 胜率
            ...
            tmp = pd.DataFrame(
                [ir, float((strategy_cum).tail(1)), return_year, return_avg, volatility, profit_max, loss_max,
                 num_of_up, num_of_down,num_of_unchange, win_rate, gain_of_up, loss_of_down, profit_loss_ratio, drawdown],
                columns=[data.name],
        ...
        if day:
            ...
            return pd.concat(res, axis=1, join='inner').drop(['年化收益率','年化波动率'])
if __name__ == '__main__':
    strategy_return=[0.1,0.9,-0.5,0.2,-0.82,0.2,-0.1,-0.1]
    result = ratio().cal_ratio(pd.DataFrame(strategy_return),day=False).round(4)
