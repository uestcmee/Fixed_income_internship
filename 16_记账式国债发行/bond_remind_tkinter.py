...
def main_fun(info_0):
    ...
    try:
        ...
        if info_now != info_0:  # 有差异
            ...
            time.sleep(0)  # 获取完成后立即更新
            return info_0,'change'
        else:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        info_0,state_flag=main_fun(info_0) # 主函数，更新比较基准
        # 延时5s ,延时放外面只用写一个函数，但是要传参
        if state_flag=='change':
            pass # 立即进入下一个循环
        else:
            for i in range(5, 0, -1):# 延时
                now_time = str(datetime.datetime.now())[:19]
                print('\r{}  {}s后再次获取  {}'.format(state_flag,i,now_time), end='')
                time.sleep(1)