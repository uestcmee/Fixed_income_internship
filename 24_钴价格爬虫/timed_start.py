...
class Timing:
    def __init__(self,title='同业成交爬虫',rule='weekday==3 and hour>9 and hour <=18'):
    ...
    def start_tk_info(self):
        ...
        tk.Label(self.window,text='定时标准：{}'.format(self.rule)).grid(row=1,column=0,columnspan=2)
        ...
        self.text_result.grid(row=2,column=1)
    ...
    def sleep(self,wait_time:int):
        ...
        for i in range(wait_time, 0, -1):  # 延时
            ...
            if self.th.is_alive() and hasattr(self,'text') : # 前面也是为了确保已经初始化完成
                ...
                if self.text != None: # 有时候运行到这里的时候窗口被重置了，如果重置会报错
...
if __name__ == '__main__':
    print('成交数据每日爬虫')
    # 工作日获取债券信息
    Timing(rule='weekday<5 and tim>"20:11:00"').main_loop(func=fetch_bond_info,wait_time=1)