...
class StgReturn:
    def __init__(self,file_name=None,l_fac=1,s_fac=1,year_range=range(2006,2021),l_range = range(12, 31, 2),s_range = range(2, 12)):
        # 在此修改长短期均线长度
        # ========================
    ...
    def main_loop(self):
        ...
        for long in self.l_range:
            for short in self.s_range:
                ...
                # self.ret_list[str(self.long) + '_' + str(self.short)] = y5_r
                print('{}_{}:{}'.format(long, short, y5_r))
        self.output(self.ret_list, out_file=str(self.l_fac) + '_' + str(self.s_fac) + '.csv')
